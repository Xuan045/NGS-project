...
PIPELINE="/staging/biology/u4432941/SRA/deepvariant.sh"
DAY=`date +%Y%m%d`
...
while read -r ID;
    ...
    rsync ${PIPELINE} ./${DAY}_deepvariant_${ID}.sh
    sed -i 's|WKDIR|'${wkdir}'|g' ./${DAY}_deepvariant_${ID}.sh
    sed -i 's|SAMPLE_ID|'${ID}'|g' ./${DAY}_deepvariant_${ID}.sh
    sbatch ./${DAY}_deepvariant_${ID}.sh
